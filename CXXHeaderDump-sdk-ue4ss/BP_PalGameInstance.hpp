...
class UBP_PalGameInstance_C : public UPalGameInstance
{
    ...
    class ULoadingProcessTask* CurrentLoadingTask;                                    // 0x09C8 (size: 0x8)
    class UPalHUDDispatchParameter_FadeWidget* LoadingFadeParameter;                  // 0x09D0 (size: 0x8)
    TSoftObjectPtr<UWorld> LoadWorld;                                                 // 0x09D8 (size: 0x30)
    bool ShowedCharaMake;                                                             // 0x0A08 (size: 0x1)
    TSoftClassPtr<APalCutsceneActor> OpeningActor;                                    // 0x0A10 (size: 0x30)
    FName CutsceneName;                                                               // 0x0A40 (size: 0x8)
    class APalCutsceneActor* OpeningCutscene;                                         // 0x0A48 (size: 0x8)
    FName flagName;                                                                   // 0x0A50 (size: 0x8)
    int32 PlayingID;                                                                  // 0x0A58 (size: 0x4)
    FDataTableRowHandle CreateSessionErrorMsgId;                                      // 0x0A60 (size: 0x10)
    FDataTableRowHandle CreateSessionErrorMsgId_Console;                              // 0x0A70 (size: 0x10)
    bool IsStartingGame;                                                              // 0x0A80 (size: 0x1)
    FName UIInputBlockFlagName;                                                       // 0x0A84 (size: 0x8)
    FTimerHandle PollingFadeInWaitHandle;                                             // 0x0A90 (size: 0x8)
