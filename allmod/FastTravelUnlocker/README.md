...
## Разблокировка АСИНХРОННА — это и было главной ловушкой

`OnTriggerInteract` запускает катсцену, и флаг `FastTravelPointUnlockFlag` выставляется
**через секунды**. В логах это видно так: сразу после вызова `IsUnlocked()=false`, а через
~20 секунд уже `IsEnableFastTravel()=true` и в `Items` появляется ключ. Проверять итог
сразу — значит гарантированно видеть ноль (мы так и прозевали настоящий успех).

Поэтому:

* `CONFIG.AsyncUnlock = true` — мод не проверяет результат сразу, а только отправляет запросы;
* `UnlockBatchSize = 1`, `UnlockBatchDelayMs = 750` — по одной точке, чтобы не рвать игру
  десятками одновременных катсцен (на пачках 3/350 мс игра падала на ~150-й точке);
* итог считается отдельно, через `FinalCheckDelays = { 5000, 15000 }` — строки
  `ПОВТОРНАЯ ПРОВЕРКА через N мс` и `+++ Реально открылось N точек`;
* `RestoreDelayMs = 25000` — в режиме `noexp` EXP rate держится нулевым на время
  асинхронных катсцен.

Ключ флага — GUID `LevelObjectInstanceId` в **верхнем регистре** (32 hex), подтверждено опытно:
`OnUpdateFlagMapRecord('479A228C4032AAF144B3808E753E1690', true)` открывает визуальное
состояние статуи, а по `FastTravelPointID` — нет.
