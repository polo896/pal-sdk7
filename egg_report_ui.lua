...
local function ellipsize(str, maxChars)
    ...
    local targetChars = math.max(1, maxChars - 2)
    local curChar = 0
    i = 1
    local cutByte = 1

    while i <= byteLen and curChar < targetChars do
        curChar = curChar + 1
        local b = str:byte(i)
        local step = 1
        if b >= 0xC0 and b <= 0xDF then
    ...
    return str:sub(1, cutByte) .. ".."
end

local function safeFText(str)
    local s = tostring(str or "")
    local ok, res = pcall(FText, s)
    if ok and res then return res end

    local sanitized = s:gsub("[\128-\255]", "?")
    local okSanitized, resSanitized = pcall(FText, sanitized)
    if okSanitized and resSanitized then return resSanitized end

    return safeCall(FText, "")
end
...
function PalUI.Factory.CreateText(tree, message, fontSize, colorTuple, boldFont, justify)
    ...
    local textObj = safeFText(message or "")
    pcall(function() widget:SetText(textObj) end)
...
local function palFingerprint(pal)
    local passives = {}
    for _, s in ipairs(pal.passiveSkills or {}) do passives[#passives + 1] = tostring(s) end
    table.sort(passives)
    return string.format("%s|%d|%d|%s|%d|%d|%d|%d|%s",
        pal.characterId or "", pal.level or 1, pal.rank or 0,
        pal.isRarePal and "R" or "-",
        pal.talentHP or 0, pal.talentMelee or 0, pal.talentShot or 0, pal.talentDefense or 0,
        table.concat(passives, ","))
end

local function slotFingerprint(save)
    if not save then return nil end
    local passives = {}
    local list = save.PassiveSkillList
    if list then
        local n = safeCall(function() return #list end) or safeCall(function() return list:GetArrayNum() end) or 0
        for i = 1, n do
            local el = safeCall(function() return list[i] end) or safeCall(function() return list:Get(i - 1) end)
            local s = el and safeCall(function() return el:ToString() end)
            if s and s ~= "" and s ~= "None" then passives[#passives + 1] = s end
        end
    end
    table.sort(passives)
    return string.format("%s|%d|%d|%s|%d|%d|%d|%d|%s",
        safeCall(function() return save.CharacterID:ToString() end) or "",
        tonumber(safeCall(function() return save.Level end)) or 1,
        tonumber(safeCall(function() return save.Rank end)) or 0,
        (safeCall(function() return save.IsRarePal end) == true) and "R" or "-",
        tonumber(safeCall(function() return save.Talent_HP end)) or 0,
        tonumber(safeCall(function() return save.Talent_Melee end)) or 0,
        tonumber(safeCall(function() return save.Talent_Shot end)) or 0,
        tonumber(safeCall(function() return save.Talent_Defense end)) or 0,
        table.concat(passives, ","))
end

local function slotFavIndex(slot)
    local param = safeCall(function() return slot:GetHandle():TryGetIndividualParameter() end)
            or safeCall(function() return slot.ReplicateIndividualParameter end)
    if not param then return nil end
    local v = tonumber(safeCall(function() return param:GetFavoriteIndex() end))
    if v == nil then v = tonumber(safeCall(function() return param.SaveParameter.FavoriteIndex end)) end
    return v or 0
end

function FavService.BuildIndex(ctx)
    if not ctx then return end
    local storage = isObjectValid(ctx.playerState) and safeCall(function() return ctx.playerState:GetPalStorage() end)
    if not isObjectValid(storage) and isObjectValid(ctx.utility) then
        storage = safeCall(function() return ctx.utility:GetLocalPalStorageData(ctx.world) end)
    end
    local container = isObjectValid(storage) and safeCall(function() return storage.TargetContainer end)
    if not isObjectValid(container) then return end

    local idx = {}
    local total = safeCall(function() return container:Num() end) or 0
    for i = 0, total - 1 do
        local slot = safeCall(function() return container:Get(i) end)
        if isObjectValid(slot) and safeCall(function() return slot:IsEmpty() end) ~= true then
            local param = safeCall(function() return slot:GetHandle():TryGetIndividualParameter() end)
                    or safeCall(function() return slot.ReplicateIndividualParameter end)
            local fp = param and slotFingerprint(param.SaveParameter)
            if fp and not idx[fp] then idx[fp] = slot end
        end
    end
    FavService.idx = idx
end

function FavService.Resolve(pal, ctx, forceSearch)
    if pal._favSlot and isObjectValid(pal._favSlot) then
        pal._favIndex = slotFavIndex(pal._favSlot) or 0
        return true
    end
    if forceSearch or not pal._favSearched then
        if not FavService.idx then FavService.BuildIndex(ctx) end
        pal._favSearched = true
        local slot = FavService.idx and FavService.idx[palFingerprint(pal)]
        if isObjectValid(slot) then
            pal._favSlot = slot
            pal._favIndex = slotFavIndex(slot) or 0
            return true
        end
    end
    return false
...
function FavService.SetIndex(pal, ctx, targetIdx)
    if not isObjectValid(pal._favSlot) then
        FavService.BuildIndex(ctx)
        FavService.Resolve(pal, ctx, true)
    end
    if not isObjectValid(pal._favSlot) then return nil end
    if not isObjectValid(FavService.uiUtil) then
        FavService.uiUtil = safeCall(function() return StaticFindObject("/Script/Pal.Default__PalUIUtility") end)
    end
    if not isObjectValid(FavService.uiUtil) then return nil end

    local cur = pal._favIndex or 0
    targetIdx = targetIdx or 0
    if cur == targetIdx then
        targetIdx = 0
    end

    local steps = (targetIdx - cur) % 4
    for _ = 1, steps do
        safeCall(function() FavService.uiUtil:ToggleFavoritePalBySlot(ctx.world, pal._favSlot) end)
    end

    pal._favIndex = targetIdx
    return targetIdx
end

function FavService.Cycle(pal, ctx)
    local cur = pal._favIndex or 0
    local nextIdx = (cur + 1) % 4
    return FavService.SetIndex(pal, ctx, nextIdx)
end

function FavService.Toggle(pal, ctx)
    if not isObjectValid(pal._favSlot) then
        FavService.BuildIndex(ctx)
        FavService.Resolve(pal, ctx, true)
    end
    ...
    local old = pal._favIndex or 0
    safeCall(function() FavService.uiUtil:ToggleFavoritePalBySlot(ctx.world, pal._favSlot) end)
    local newIdx = slotFavIndex(pal._favSlot)
    if newIdx == nil or newIdx == old then
        newIdx = (old + 1) % 4
    end
    pal._favIndex = newIdx
    return newIdx
end

function FavService.Reset()
    FavService.idx = nil
end
...
local createGameButton, applyView, rebuildList
...
local function drawPalCard(scrollBox, tree, pal, cardW)
    ...
    local ctx = ControllerState.ctx
    if ctx then FavService.Resolve(pal, ctx) end
    local favIdx = pal._favIndex or 0

    local btnW, btnH = 30, 22
    local btnX = cardW - padX - btnW
    local btnY = headY - 1

    local function getFavText(idx)
        if idx == 1 then return "★1"
        elseif idx == 2 then return "★2"
        elseif idx == 3 then return "★3"
        else return "★" end
    end

    local starTxt = F.CreateText(
        tree,
        getFavText(favIdx),
        11,
        (favIdx > 0) and T.Gold or withAlpha(T.TextDim, 0.40),
        favIdx > 0,
        1
    )

    if starTxt then
        F.AnchorWidget(card, starTxt, btnX, btnY + 2, btnW, 18, 7)
        pcall(function() starTxt:SetVisibility(3) end)
    end

    local function updateStar(idx)
        if not isObjectValid(starTxt) then return end
        local isFav = (idx > 0)
        local col = isFav and T.Gold or withAlpha(T.TextDim, 0.40)
        local txt = getFavText(idx)
        pcall(function()
            starTxt:SetText(safeFText(txt))
            local c = starTxt.ColorAndOpacity
            c.SpecifiedColor.R, c.SpecifiedColor.G = col[1], col[2]
            c.SpecifiedColor.B, c.SpecifiedColor.A = col[3], col[4] or 1.0
            if not pcall(function() starTxt:SetColorAndOpacity(c) end) then
                starTxt.ColorAndOpacity = c
            end
        end)
    end

    createGameButton(ControllerState.hostCanvas, card, tree, "",
        btnX, btnY, btnW, btnH,
        function()
            local newIdx = FavService.Toggle(pal, ControllerState.ctx)
...
function rebuildList()
    ...
    if isObjectValid(S.pageLabel) then
        ...
        pcall(function() S.pageLabel:SetText(safeFText(label)) end)
    end
...
function createGameButton(hostCanvas, surface, tree, label, x, y, w, h, onClick, z, parent)
    ...
    local ok, btn = pcall(function()
        ...
        local b = widgetLib:Create(parent or hostCanvas, btnCls, owner)
        if not isObjectValid(b) then error("button create failed") end
        pcall(function() b:Setup(false) end)
        pcall(function() b:SetText(safeFText(label)) end)
        return b
    end)
    if not ok or not isObjectValid(btn) then return nil end

    PalUI.Factory.AnchorWidget(parent or surface, btn, x, y, w, h, z or 60)
    ...
    S.sortButton = createGameButton(hostCanvas, surface, tree, SORT_LABELS[1],
        ...
        function()
            ...
            if isObjectValid(st.sortButton) then
                pcall(function() st.sortButton:SetText(safeFText(SORT_LABELS[st.sortMode])) end)
            end
            applyView()
            rebuildList()
        end, 60)
