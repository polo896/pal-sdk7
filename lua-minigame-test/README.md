...
> **v0.2 — фикс после первого теста.** В v0.1 объекты из колбэков хуков приходили обёрнутыми в
> `RemoteUnrealParam`, из-за чего класс параметра не определялся («мини-игра началась: ?») и
> победа не вызывалась. В v0.2 все объекты разворачиваются через `:get()` (тот же паттерн, что
> в PalWarp/Relic Collector), классификация параметра идёт 4 уровнями (GetClass → имя объекта →
> проба членов → проба вызова), дедуп победы — по полному имени объекта. Если параметр всё же
> не опознается — в лог упадёт блок `ДИАГНОСТИКА`, пришли его целиком.
